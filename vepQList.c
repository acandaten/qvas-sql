...
void q_list_insert(QList *obj, int idx, void *item) {
  ...
  list_check_size(obj, obj->size + 1);
  if (idx > q_list_size(obj))
    idx = q_list_size(obj);
  size_to_move = (obj->size - idx);

  memmove(&(obj->data[idx + 1]), &(obj->data[idx]),
          sizeof(void *) * size_to_move);
  obj->data[idx] = item;
  obj->size++;
};
...
void *q_list_delete(QList *obj, int idx) {
  void *out = NULL;
  int size_to_move = 0;
  if (idx < 0 || idx >= q_list_size(obj))
    return out;
  size_to_move = (q_list_size(obj) - idx - 1);
  out = q_list_get(obj, idx);
  memmove(&(obj->data[idx]), &(obj->data[idx + 1]),
          sizeof(void *) * size_to_move);
  obj->size--;
  return out;
};
...
@specfun
 +-----------------------------------------------------*/
int q_list_size(QList *obj) { return obj->size; };
